...
for ((i = 1; i <=${BATCH}; i++));do
  mysql -hcontentdb.localhost.rds.cn-north-1.amazonaws.com.cn -p3306 -udatalab -pdatalab analytics_company_data --execute "${SQL} WHERE puid>${AUTO_ID} LIMIT ${DATA_SIZE}) a WHERE a.name IS NOT NULL AND a.name<>''"  -s |sed -e 's/NULL/  /g' -e 's/\t/","/g;s/^/"/;s/$/"/;s/\n//g' >> ${CSV_FILE_NAME}
...
for ((i = 1; i <=${BATCH}; i++));do
  ...
  mysql -hcontentdb.localhost.rds.cn-north-1.amazonaws.com.cn -p3306 -udatalab -pdatalab analytics_master_data --execute "SELECT hcode AS name,SUBSTRING(hcode,LENGTH('HORG')+1,LENGTH(hcode)) AS code,'HORG' AS prefix_code,CONVERT(DATE_FORMAT(hupdatetime,'%Y%m%d%H%i%S'),SIGNED INTEGER) AS hupdatetime,hisvalid FROM analytics_master_data.MSTR_ORG WHERE huid>=${AUTO_MIN} AND huid<${AUTO_MAX}"  -s |sed -e 's/NULL/  /g' -e 's/\t/","/g;s/^/"/;s/$/"/;s/\n//g' >> ${CSV_FILE_NAME}
  AUTO_MIN=${AUTO_MAX}
  AUTO_MAX=`expr ${AUTO_MIN} + ${DATA_SIZE}`
done
