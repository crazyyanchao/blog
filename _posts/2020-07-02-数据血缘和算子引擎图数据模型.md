...
## 通过指标ID获取关联的数据血缘图谱
```
MATCH (n:MySQLDBTBField) WHERE ANY(indicator IN apoc.convert.fromJsonList(n.indicator_ids) WHERE indicator='HIDSC1')
CALL apoc.path.expand(n,NULL,'数据血缘',0,3) YIELD path RETURN path
```
## 通过指标ID获取关联的指标算子图谱
```
MATCH (n:算子) WHERE ANY(indicator IN apoc.convert.fromJsonList(n.indicator_ids) WHERE indicator='HIDSC1')
CALL apoc.path.expand(n,NULL,'算子引擎',0,3) YIELD path RETURN path
```
