...
5、寻找衍生字段【MATCH (n:MySQLDBTBField) WHERE SIZE((n)<--(:MySQLDBTBField))>1 RETURN n】
6、寻找原始字段【MATCH (n:MySQLDBTBField) WHERE SIZE((n)<--(:MySQLDBTBField))=0 RETURN n】
7、寻找衍生表【MATCH (n:MySQLDBTBField) WHERE SIZE((n)<--(:MySQLDBTBField))>1 WITH n MATCH (n)<-[:包含]-(m:MySQLDBTB) RETURN m】
8、寻找原始表【MATCH (n:MySQLDBTBField) WHERE SIZE((n)<--(:MySQLDBTBField))=0 WITH n MATCH (n)<-[:包含]-(m:MySQLDBTB) RETURN m】

## 算子引擎全景图相关的指标
1、获取数据的算子【MATCH (n:算子) WHERE n.is_get_data=true RETURN n】
2、计算算子【MATCH (n:算子) WHERE n.is_get_data=false RETURN n】
3、指标类型相关的算子统计【给SQL】
...
5、被算子API使用的字段【MATCH (n:算子)<-[:获取数据]-(m:MySQLDBTBField) WHERE n.is_get_data=true RETURN m】
